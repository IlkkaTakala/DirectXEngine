...
struct Key
{
	...
	Key(unsigned int code, DeviceType type = DeviceType::Keyboard, InputMappingType dataType = InputMappingType::Button, TriggerType trigger = TriggerType::Released)
...
public:
	...
	static void RegisterInputComponent(InputComponent* component, User user);
	static void UnregisterInputComponent(InputComponent* component);
	...
	static void MakeInputMapping(const std::string& name, std::map<std::string, std::tuple<std::vector<Key>, bool>>&& data);
	static void SetUserMapping(User user, const std::string& name);
	...
	static int ActionToActionID(const std::string& action);
...
private:
	...
	static std::map<std::string, int> ActionToID;
	static std::map<std::string, InputMappings> AllinputMappings;
	static std::map<User, UserDeviceData> Users;
};
