...
std::map<std::string, int> InputManager::ActionToID = {};
std::map<std::string, InputMappings> InputManager::AllinputMappings = {};
std::map<User, UserDeviceData> InputManager::Users = {};
...
void InputManager::Update()
{
	...
	// Only first player has keyboard
	for (auto& [user, data] : Users) {
		for (auto& [action, keys] : data.Map->Keys) {
			if (data.flushing) break;
			XMFLOAT2 value{};
			for (auto& key : std::get<0>(keys)) {
				switch (key.Type) {
				case DeviceType::Keyboard: {
					if (IsKeyboardKey((InputState)key.Trigger, key.KeyCode))
						switch (key.DataType)
						{
				...
				case DeviceType::Controller: {
					...
					case InputMappingType::Axis2D:
						auto val = GetThumbstickPosition(key.KeyCode == 0, (GamepadIndex)user);
						value.x += val.x;
						value.y += val.y;
						break;
					case InputMappingType::Axis:
						value.x += GetTriggerPressure(key.KeyCode == 0, (GamepadIndex)user);
						break;

					default:
						if (IsGamepadButton((InputState)key.Trigger, (WORD)key.KeyCode, (GamepadIndex)user)) {
							switch (key.DataType)
							{
							case InputMappingType::Button:
							{
				...
				case DeviceType::Mouse: {
					switch (key.DataType) {
					case InputMappingType::Axis2D:
						auto p = GetMouseMovement();
						value.x += p.x;
						value.y += p.y;
						break;
					case InputMappingType::Button:
						value.x += IsMouseButton((InputState)key.Trigger, key.KeyCode) ? 1.f : 0.f;
						break;
					default:
						break;
					}
			...
			float len;
			auto v = XMLoadFloat2(&value);
			XMStoreFloat(&len, XMVector2Length(v));
			if (len > 0.1) {
				if (std::get<1>(keys)) XMStoreFloat2(&value, v / len);
				for (auto& in : Users[0].InputStack) {
					in->TriggerAction(action, value);
				}
			}
		}
		data.flushing = false;
	...
	ForceMouseToCenter(m_ForceToCenter);
}
...
void InputManager::MakeInputMapping(const std::string& name, std::map<std::string, std::tuple<std::vector<Key>, bool>>&& data)
{
	InputMappings map;
...
void InputManager::SetUserMapping(User user, const std::string& name)
{
...
int InputManager::ActionToActionID(const std::string& action)
{
	if (auto it = ActionToID.find(action); it != ActionToID.end()) {
		return it->second;
	}
...
void InputManager::RegisterInputComponent(InputComponent* component, User user)
{
	Users[user].InputStack.remove(component);
	Users[user].InputStack.push_back(component);
}

void InputManager::UnregisterInputComponent(InputComponent* component)
{
