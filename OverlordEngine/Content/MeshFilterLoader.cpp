...
#pragma region Animation Blocks
				...
				case OVM_HEADER_ANIMATIONS::SKELETON:
				{
					...
					pMeshFilter->Bones.resize(pMeshFilter->m_BoneCount);

					for (int i = 0; i < pMeshFilter->m_BoneCount; ++i) {
						pMeshFilter->Bones[i].id = pReader->Read<UINT16>();
						pReader->ReadString();
						pMeshFilter->Bones[i].parent = pReader->Read<UINT16>();
						pMeshFilter->Bones[i].offset = pReader->Read<XMFLOAT4X4>();
						pMeshFilter->Bones[i].localTransform = pReader->Read<XMFLOAT4X4>();
						pMeshFilter->Bones[i].worldTransform = pReader->Read<XMFLOAT4X4>();
					}
				}
				break;
				}
#pragma endregion
	...
	//	for (auto& a : pMeshFilter->m_AnimationClips) {
	//		for (auto& k : a.keys) {
	//			for (int i = (int)k.boneTransforms.size() - 1; i >= 0; --i) {
	//				XMMATRIX mat = XMLoadFloat4x4(&k.boneTransforms[i]);
	//				auto l = XMMatrixInverse(nullptr, (XMLoadFloat4x4(&pMeshFilter->Bones[i].localTransform)));
	//				auto o = XMMatrixInverse(nullptr, XMMatrixTranspose(XMLoadFloat4x4(&pMeshFilter->Bones[i].offset)));
	//				mat = XMMatrixMultiply(mat, (l));
	//				mat = XMMatrixMultiply(mat, (o));

	//				//if (pMeshFilter->Bones[i].parent >= 0)
	//				//	mat = XMMatrixMultiply(mat, XMMatrixInverse(nullptr, XMLoadFloat4x4(&k.boneTransforms[pMeshFilter->Bones[i].parent])));
	//				//

	//				XMStoreFloat4x4(&k.boneTransforms[i], mat);
	//			}
	//		}
	//	}
	//}
