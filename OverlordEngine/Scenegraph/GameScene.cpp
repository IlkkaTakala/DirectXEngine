...
#endif
	std::erase_if(Timers,
		[&pObject](const std::pair<size_t, TimerData>& item) -> bool {
			return item.second.object == pObject;
	});
...
void GameScene::RootUpdate()
{
	...
	for (auto& [handle, t] : Timers) {
		t.elapsed += m_SceneContext.pGameTime->GetElapsed();
...
#pragma warning(pop)
	...
	auto children = m_pChildren;
	for (const auto pChild : children)
	{
		if (pChild->m_PendingDestroy) {
...
#pragma region SHADOW PASS
	...
	for (auto& c : m_pChildren) {
		c->RootShadowMapDraw(m_SceneContext);
	}
...
#pragma region POST-PROCESSING PASS
	...
	//No need to swap RenderTargets is there aren't any PP Effects...
	{
		...
		auto prevRT = m_SceneContext.useDeferredRendering ? DeferredRenderer::Get()->GetBuffer() : m_pGame->GetRenderTarget();
		...
		for (auto& m : m_PostProcessingMaterials) {
			if (!m->IsEnabled()) continue;
			m->Draw(m_SceneContext, prevRT);
			prevRT = m->GetOutput();
		}
		...
		if (prevRT != m_pGame->GetDefaultRenderTarget()) {
			m_pGame->SetRenderTarget(nullptr);
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, prevRT->GetColorShaderResourceView(), {});
...
Timer GameScene::SetTimerByEvent(GameObject* object, float time, const TimerCallback& callback, bool looping)
{
	size_t id{ 0 };
	if (!FreeList.empty()) {
		id = FreeList.front();
...
float GameScene::GetRemainingTime(const Timer& handle)
{
...
float GameScene::GetElapsedTime(const Timer& handle)
{
	if (auto it = Timers.find(handle); it != Timers.end()) {
...
void GameScene::ClearTimer(const Timer& handle)
{
	Timers.erase(handle);
	FreeList.push(handle);
}

void GameScene::ClearAllTimers()
{
	Timers.clear();
...
ImFont* GameScene::GetFont(const std::string& name)
{
	return m_pGame->GetGameContext().Fonts.find(name)->second;
}
