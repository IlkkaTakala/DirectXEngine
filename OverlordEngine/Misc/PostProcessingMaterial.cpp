...
void PostProcessingMaterial::UpdateBaseEffectVariables(const SceneContext& /*sceneContext*/, RenderTarget* pSource)
{
	...
	m_pBaseEffect->GetVariableByName("gNormalTexture")->AsShaderResource()->SetResource(DeferredRenderer::Get()->GetNormalBuffer()->GetColorShaderResourceView());
	m_pBaseEffect->GetVariableByName("gBloomTexture")->AsShaderResource()->SetResource(DeferredRenderer::Get()->GetBloomBuffer()->GetColorShaderResourceView());
