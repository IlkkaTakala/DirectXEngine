...
HRESULT OverlordGame::InitializeWindow()
{
	...
	if (m_GameContext.fullscreen) {
		...
		if (GetMonitorInfo(hmon, &mi)) {
			x = mi.rcMonitor.left;
			y = mi.rcMonitor.top;
			winWidth = mi.rcMonitor.right - mi.rcMonitor.left;
			winHeight = mi.rcMonitor.bottom - mi.rcMonitor.top;

			m_GameContext.windowHeight = winHeight;
			m_GameContext.windowWidth = winWidth;
		}
	}

	DWORD style = m_GameContext.fullscreen ? WS_POPUP | WS_VISIBLE : WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
...
void OverlordGame::AddFont(const std::string& name, ImFont* font)
{
	m_GameContext.Fonts.emplace(name, font);
}
...
LRESULT OverlordGame::WindowsProcedure(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		case WM_DESTROY:
			...
			return 0;
		case WM_ACTIVATE:
			if (wParam == WA_ACTIVE || wParam == WA_CLICKACTIVE) { StateChanged(1, true); StateChanged(2, true); }
			else {
				StateChanged(1, false);
				StateChanged(2, false);
			}
			return 0;
		case WM_SIZE:
...
RenderTarget* OverlordGame::GetDefaultRenderTarget() const
{
