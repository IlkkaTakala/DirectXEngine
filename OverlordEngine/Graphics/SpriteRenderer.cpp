...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		HRESULT res = sceneContext.d3dContext.pDevice->CreateBuffer(&vertexBuffDesc, nullptr, &m_pVertexBuffer);
		HANDLE_ERROR(res);
	...
	if (m_pVertexBuffer)
	{
		...
		sceneContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
		memcpy(map.pData, m_Sprites.data(), sizeof(VertexSprite) * m_BufferSize);
