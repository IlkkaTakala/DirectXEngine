...
void DeferredRenderer::Initialize()
{
	...
	const auto pDefaultRenderTarget = CreateRenderTarget(w, h, DXGI_FORMAT_R32G32B32A32_FLOAT, true);
	m_pDefaultRenderTargetView = pDefaultRenderTarget->GetRenderTargetView(); //Main ATV
	...
	m_GBuffer[int(eGBufferId::Bloom)] = CreateRenderTarget(w, h, DXGI_FORMAT_R32G32B32A32_FLOAT);
...
RenderTarget* DeferredRenderer::CreateRenderTarget(UINT width, UINT height, DXGI_FORMAT format, bool enableDepth) const
{
	...
	rtDesc.enableDepthSRV = enableDepth;
...
void DeferredRenderer::End(const SceneContext& sceneContext) const
{
	...
	rTargets[1] = m_GBuffer[int(eGBufferId::Bloom)]->GetRenderTargetView();
	...
	m_pLightPassRenderer->VolumetricLightPass(sceneContext, m_ShaderResourceViews, rTargets, 2);
	...
	if (sceneContext.pSky)
		sceneContext.pSky->RenderSky(sceneContext);

	//6. Reset Game RenderTarget (back to normal)
	pDeviceContext->OMSetRenderTargets(RT_COUNT, rTargets, m_pDefaultDepthStencilView);
