...
void DeferredLightRenderer::DirectionalLightPass(const SceneContext& sceneContext, ID3D11ShaderResourceView* const gbufferSRVs[]) const
{
	...
	if(light.isEnabled)
	{
		...
		const auto shadow = ShadowMapRenderer::Get();
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gShadowMap", shadow->GetShadowMap());
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gFarShadowMap", shadow->GetFarShadowMap());
		m_pDirectionalLightMaterial->SetVariable_Matrix(L"gViewProj_Light", shadow->GetLightVP());
		m_pDirectionalLightMaterial->SetVariable_Matrix(L"gViewProj_LightFar", shadow->GetFarLightVP());
		auto size = shadow->GetSize();
		m_pDirectionalLightMaterial->SetVariable_Vector(L"gShadowSize", &size.x);
...
void DeferredLightRenderer::VolumetricLightPass(const SceneContext& sceneContext, ID3D11ShaderResourceView* const gbufferSRVs[], ID3D11RenderTargetView* pDefaultRTV[], int RTVCount) const
{
	//Set DefaultRTV WITH Read-Only DSV (no write access to DepthBuffer, but still able to write to StencilBuffer)
	sceneContext.d3dContext.pDeviceContext->OMSetRenderTargets(RTVCount, pDefaultRTV, m_pReadOnlyDepthStencilView);
	...
	m_pVolumetricLightMaterial->SetVariable_Vector(L"gEyePos", sceneContext.pCamera->GetTransform()->GetPosition());
