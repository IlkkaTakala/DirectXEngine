...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col, float2x2 uvRotation)
{
	...
	data.Position = mul(float4(pos, 1.0), gWorldViewProj);
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	topLeft = origin + mul(float3(size, -size, 0.0), (float3x3)gViewInverse) * 0.5;
	topRight = origin + mul(float3(-size, -size, 0.0), (float3x3)gViewInverse) * 0.5;
	bottomLeft = origin + mul(float3(size, size, 0.0), (float3x3)gViewInverse) * 0.5;
	bottomRight = origin + mul(float3(-size, size, 0.0), (float3x3)gViewInverse) * 0.5;
