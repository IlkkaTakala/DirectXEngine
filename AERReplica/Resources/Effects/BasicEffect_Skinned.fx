...
VS_Output MainVS(VS_Input input) {
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        p += input.BoneWeights[i] * mul(float4(input.Position, 1.0), gBones[input.BoneIndices[i]]);
        norm += input.BoneWeights[i] * mul(input.Normal, (float3x3) gBones[input.BoneIndices[i]]);
        norm += input.BoneWeights[i] * mul(input.Tangent, (float3x3) gBones[input.BoneIndices[i]]);
    }
    lastWeight = 1.0f - lastWeight;

    p += lastWeight * mul(float4(input.Position, 1.0), gBones[input.BoneIndices[3]]);
    norm += lastWeight * mul(input.Normal, (float3x3) gBones[input.BoneIndices[3]]);
    norm += lastWeight * mul(input.Tangent, (float3x3) gBones[input.BoneIndices[3]]);
	
	output.Position = mul(float4(p.xyz, 1.0f), gWorldViewProj);

	output.Normal = normalize(mul(norm, (float3x3)gWorld));
	output.Tangent = normalize(mul(tang, (float3x3)gWorld));
...
PS_Output MainPS(VS_Output input){
	...
	if (0) {
		...
		//normal = input.Normal;// normalize(lerp(normal, input.Normal, input.Position.z / input.Position.w));
